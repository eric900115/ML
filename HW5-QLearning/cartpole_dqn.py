...
class DQN(object):
    def __init__(self, n_states, n_actions, n_hidden, batch_size, lr, epsilon, gamma, target_replace_iter, memory_capacity):
        ...
        self.eval_net, self.target_net = Net(
            n_states, n_actions, n_hidden), Net(n_states, n_actions, n_hidden)
    ...
    def learn(self):
        ...
        b_action = torch.tensor(
            b_memory[:, self.n_states: self.n_states + 1], dtype=torch.long)
        b_reward = torch.tensor(
            b_memory[:, self.n_states + 1: self.n_states + 2], dtype=torch.float)
        b_next_state = torch.tensor(
            b_memory[:, -self.n_states:], dtype=torch.float)
...
epsilon = 0.2               # epsilon-greedy, factor to explore randomly
gamma = 0.99                 # reward discount factor
target_replace_iter = 500   # target network update frequency
